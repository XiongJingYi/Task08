...
for(var i=0;i<btn.length;i++){
	btn[i].onclick=function(){
		...
		liNode.setAttribute("class","things");
		liNode.setAttribute("value",liCount);
		list.appendChild(liNode);
		//console.log(liNode.getAttribute("value"));

		for(var j=0;j<things.length;j++){
			things[j].onclick=function(){
...
for(var i=0;i<quickview.length;i++){
    //quickview
 	views[i].onclick=function(){
		...
		var openUrl ="E:\\D2\\web2\\views\\viewss\\view"+t+".html";//弹出窗口的url
...
Delete.onclick=function(){
	if(list.hasChildNodes()){
		list.removeChild(things[n-1]);
		for(var i=0;i<things.length;i++){
			things[i].setAttribute("value",i+1);
		}
		num--;
		count.setAttribute("value",num);
